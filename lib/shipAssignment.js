...
let draggedCrewId = null;
let draggedShipCardId = null;
let teamMembers = [];
...
// ============================================================================
// TRANSIENT STATE (Session-only, cleared on app close)
// ============================================================================
...
/**
 * Sets the current alert name (transient - only in memory)
 * @param {string} name - The name/identifier of the alert
 */
function setCurrentAlertName(name) {
    currentAlertName = name || null;
    alertStartTime = name ? Date.now() : null;
    console.log('Alert name set to:', currentAlertName);
}

/**
 * Gets the current alert name
 * @returns {string|null} The current alert name or null
 */
function getCurrentAlertName() {
    return currentAlertName;
}

/**
 * Gets the alert start time
 * @returns {number|null} The timestamp when alert was started
 */
function getAlertStartTime() {
    return alertStartTime;
}

/**
 * Clears the current alert name
 */
function clearCurrentAlertName() {
    currentAlertName = null;
    alertStartTime = null;
    console.log('Alert name cleared');
}
...
function mapTeamRoleToCode(role) {
    const normalized = normalizeName(role);
    if (!normalized) return null;

    if (normalized.includes("pilot")) return "PIL";
    if (normalized.includes("teamlead") || normalized.includes("lead")) return "LEAD";
    if (normalized.includes("med")) return "MED";
    if (normalized.includes("cap") || normalized.includes("combat") || normalized.includes("aerospace")) return "CAP";
    if (normalized.includes("security")) return "SEC";
    if (normalized.includes("member")) return "SEC";
    return null;
}
...
function findTeamMemberByName(name) {
    ...
    return teamMembers.find((member) => normalizeName(member.rsiHandle) === target);
}
...
// PERSISTENCE
// ============================================================================
// NOTE: Ship assignments are NO LONGER automatically saved on every change.
// They only exist in memory during the current session and are cleared when
// the app is closed. This can be changed if needed by calling saveShipAssignments()
...
function saveShipAssignments() {
    try {
        ...
        console.log("Ship assignments saved to storage");
    } catch (e) {
    ...
    try {
        ...
        if (stored) {
            ...
            if (data.ships && Array.isArray(data.ships)) {
                ...
                console.log("Ship assignments loaded from storage");
                return true;
            }
        }
    } catch (e) {
...
function updateCrewPosition(shipId, crewId, position) {
    ...
    if (ship) {
        ...
        if (crew) {
            ...
            // Check if position is already taken by another crew member (across all ships)
            if (newPosition) {
                const positionTaken = ships.some(s => 
                    s.crew.some(c => c.id !== crewId && c.position === newPosition)
                );
                
                if (positionTaken) {
                    console.warn(`Position ${newPosition} is already taken`);
                    alert(`Position ${newPosition} ist bereits vergeben. Bitte wähle eine andere Position.`);
                    renderShips();
                    return;
                }
            }
            
            crew.position = newPosition;
            // Mark as manually set when a concrete position is chosen; clearing removes manual flag
            crew.positionManual = newPosition != null;
            updatePreview();
...
function updateCrewName(shipId, crewId, name) {
    ...
    if (ship) {
        ...
        if (crew) {
            ...
            if (matchedMember) {
                if (matchedMember.discordId) {
                    crew.discordId = matchedMember.discordId;
                }
                const mappedRole = mapTeamRoleToCode(matchedMember.role);
                if (mappedRole) {
                    crew.role = mappedRole;
                }

                // If not manually set, try to sync position to team order when available and free
                if (matchedMember.order && (!crew.positionManual || !crew.position)) {
                    const desiredPos = matchedMember.order;
                    const positionTaken = ships.some(s =>
                        s.crew.some(c => c.id !== crewId && c.position === desiredPos)
                    );
                    if (!positionTaken) {
                        crew.position = desiredPos;
                        crew.positionManual = false;
                    }
                }
            }
            updatePreview();
...
function updateCrewNameDatalist() {
    const nameDatalist = document.getElementById("crew-name-suggestions");
    const discordDatalist = document.getElementById("crew-discord-suggestions");
    if (!nameDatalist || !discordDatalist) return;
    ...
    ships.forEach(ship => {
        ship.crew.forEach(crew => {
            if (crew.name && crew.name.trim()) {
                assignedNames.add(normalizeName(crew.name));
            }
    ...
    const teamMemberNames = new Set(teamMembers.map(m => normalizeName(m.rsiHandle)).filter(Boolean));
    const seenNameKeys = new Set();

    teamMembers.forEach((member) => {
        if (!member || !member.rsiHandle) return;
        const normalized = normalizeName(member.rsiHandle);
        if (!normalized || assignedNames.has(normalized)) return;

        const option = document.createElement("option");
        option.value = member.rsiHandle;

        const labelParts = [];
        if (member.role) labelParts.push(member.role);
        if (member.discordId) labelParts.push(member.discordId);
        if (labelParts.length) {
            option.label = labelParts.join(" · ");
        }

        nameDatalist.appendChild(option);
        seenNameKeys.add(normalized);

        if (member.discordId && !assignedDiscordIds.has(member.discordId.trim())) {
            const discordOption = document.createElement("option");
            discordOption.value = member.discordId.trim();
            discordOption.label = member.rsiHandle;
            discordDatalist.appendChild(discordOption);
        }
    });
    ...
    cachedNames.forEach(name => {
        ...
        if (assignedNames.has(normalized)) return;
        if (seenNameKeys.has(normalized)) return;
        if (teamMemberNames.size > 0 && !teamMemberNames.has(normalized)) return;
...
function backfillCrewWithTeamData() {
    let changed = false;

    ships.forEach((ship) => {
        ship.crew.forEach((crew) => {
            const member = findTeamMemberByName(crew.name);
            if (!member) return;

            if (!crew.discordId && member.discordId) {
                crew.discordId = member.discordId;
                changed = true;
            }

            const mappedRole = mapTeamRoleToCode(member.role);
            if (mappedRole && (!crew.role || crew.role === "SEC")) {
                crew.role = mappedRole;
                changed = true;
            }
            
            // Sync position from team order when not manually overridden, or when no position is set
            if (member.order && (!crew.positionManual || !crew.position || crew.position !== member.order)) {
                const positionTaken = ships.some(s => 
                    s.crew.some(c => c.id !== crew.id && c.position === member.order)
                );
                if (!positionTaken) {
                    crew.position = member.order;
                    crew.positionManual = false;
                    changed = true;
                }
            }
...
function autoAssignCrewToShips() {
    if (!teamMembers || teamMembers.length === 0) return;
    
    // Find all assigned crew names
    const assignedNames = new Set();
    ships.forEach(ship => {
        ship.crew.forEach(crew => {
            if (crew.name && crew.name.trim()) {
    ...
    // Find unassigned team members
    const unassignedMembers = teamMembers.filter(member => {
        const normalized = normalizeName(member.rsiHandle);
        return normalized && !assignedNames.has(normalized);
    });
    ...
    unassignedMembers.forEach(member => {
        const role = mapTeamRoleToCode(member.role);
        if (!role) return;
        
        let targetShip = null;
        let shouldCreateShip = false;
        
        if (role === "PIL" || role === "LEAD") {
            // Pilot and Lead go to Gunship
            targetShip = ships.find(s => s.type === "Gunship");
            if (!targetShip) {
                shouldCreateShip = true;
                targetShip = {
                    id: shipIdCounter++,
                    type: "Gunship",
                    ship: "",
        ...
        if (targetShip) {
            ...
            const memberOrder = member.order || null;
            let assignedPosition = null;
            
            if (memberOrder) {
                // Check if this position is already used across all ships
                const positionTaken = ships.some(s => 
                    s.crew.some(c => c.position === memberOrder)
                );
                
                if (!positionTaken) {
                    assignedPosition = memberOrder;
                }
            }
            
            // Add crew member to ship
            targetShip.crew.push({
                id: Date.now() + Math.random(),
                role: role,
                position: assignedPosition,
                positionManual: false,
                name: member.rsiHandle || "",
                discordId: member.discordId || "",
                comment: ""
            });
            
            if (shouldCreateShip) {
                ships.push(targetShip);
            }
    ...
    // Ensure a Medship exists whenever a Gunship exists (place Medship between Gunship and CAP)
    const hasGunship = ships.some(s => s.type === "Gunship");
    const hasMedship = ships.some(s => s.type === "Medship");
    if (hasGunship && !hasMedship) {
        ships.push({ id: shipIdCounter++, type: "Medship", ship: "", crew: [] });
        changed = true;
    }

    // Reorder ships so Gunship(s) first, then Medship(s), then CAP(s)
    if (changed) {
        const gunships = ships.filter(s => s.type === "Gunship");
        const medships = ships.filter(s => s.type === "Medship");
        const caps = ships.filter(s => s.type === "CAP");
        const others = ships.filter(s => s.type !== "Gunship" && s.type !== "Medship" && s.type !== "CAP");
        ships = [...gunships, ...medships, ...caps, ...others];
    }
    
    if (changed) {
        console.log(`[shipAssignment] Auto-assigned ${unassignedMembers.length} crew member(s)`);
        renderShips();
        updatePreview();

        
        // Play unassigned sound if enabled
        if (typeof process !== "undefined" && process.send) {
            process.send({ type: "play-unassigned-sound" });
        }
    }
}

function autoFillShipAssignments() {
    // Reuse existing team-based auto-assign logic to populate ships quickly
    autoAssignCrewToShips();
}

function setTeamMembers(members) {
    const newMembers = Array.isArray(members) ? members : [];
    
    // Find members who left the team
    const newMemberNames = new Set(newMembers.map(m => normalizeName(m.rsiHandle)));
    const oldMemberNames = new Set(teamMembers.map(m => normalizeName(m.rsiHandle)));
    
    const leftMembers = [];
    oldMemberNames.forEach(oldName => {
        if (!newMemberNames.has(oldName)) {
            leftMembers.push(oldName);
        }
    });
    
    // Remove crew members who left the team and adjust positions
    if (leftMembers.length > 0) {
        let removed = false;
        const removedPositions = [];
        
        ships.forEach(ship => {
            const originalLength = ship.crew.length;
            
            // Collect positions of crew members being removed
            ship.crew.forEach(crew => {
                const crewName = normalizeName(crew.name);
                if (leftMembers.includes(crewName) && crew.position) {
                    removedPositions.push(crew.position);
                }
            });
            
            // Remove crew members
            ship.crew = ship.crew.filter(crew => {
                const crewName = normalizeName(crew.name);
                return !leftMembers.includes(crewName);
            });
            
            if (ship.crew.length < originalLength) {
                removed = true;
            }
        });
        
        // Adjust positions for remaining crew members
        if (removedPositions.length > 0) {
            removedPositions.sort((a, b) => a - b);
            removedPositions.forEach(removedPos => {
                ships.forEach(s => {
                    s.crew.forEach(c => {
                        if (c.position && c.position > removedPos) {
                            c.position -= 1;
                        }
                    });
                });
        ...
        if (removed) {
            console.log(`[shipAssignment] Removed crew members who left the team:`, leftMembers);
            console.log(`[shipAssignment] Adjusted positions after removal`);
            renderShips();
            updatePreview();
    ...
    updateCrewNameDatalist();
    backfillCrewWithTeamData();
    autoAssignCrewToShips();
}

async function initializeTeamMemberSync() {
    ...
    try {
        ...
        console.warn("Failed to load team members for ship assignments:", error);
    }

    if (window.api.onTeamMembersUpdate) {
...
function renderShips() {
    ...
    container.innerHTML = ships.map(ship => `
        <div class="ship-card mb-4 rounded-lg border border-gray-600 bg-gray-800 p-4" data-ship-id="${ship.id}" draggable="true" ondragstart="handleShipDragStart(event, ${ship.id})" ondragend="handleShipDragEnd(event)" ondragover="handleShipDragOver(event)" ondrop="handleShipDrop(event, ${ship.id})">
            <!-- Ship Header -->
            ...
            <!-- Crew List -->
            <div class="crew-list flex flex-col gap-2" ondragover="handleCrewDragOver(event)" ondrop="handleCrewDrop(event, ${ship.id})">
                ${ship.crew.map(crew => `
                    <div class="crew-member" draggable="true" ondragstart="handleCrewDragStart(event, ${ship.id}, ${crew.id})" ondragend="handleCrewDragEnd(event)" style="display: grid; grid-template-columns: auto auto 1fr 1fr auto; gap: 0.75rem; align-items: center; padding: 0.75rem; background: var(--surface); border: 1px solid var(--border); border-radius: 8px; cursor: move;">
                        <!-- Position -->
...
function copyToClipboard() {
    ...
    // If clipboard API is unavailable or the document is unfocused, use fallback
...
function handleCrewDragStart(event, shipId, crewId) {
    draggedShipId = shipId;
    draggedCrewId = crewId;
    event.target.style.opacity = '0.5';
}

function handleCrewDragEnd(event) {
    event.target.style.opacity = '1';
}

function handleCrewDragOver(event) {
    event.preventDefault();
    event.dataTransfer.dropEffect = 'move';
}

function handleCrewDrop(event, targetShipId) {
    event.preventDefault();
    
    if (draggedShipId === null || draggedCrewId === null) return;
    if (draggedShipId === targetShipId) return;
    
    const sourceShip = ships.find(s => s.id === draggedShipId);
    const targetShip = ships.find(s => s.id === targetShipId);
    
    if (!sourceShip || !targetShip) return;
    
    const crewIndex = sourceShip.crew.findIndex(c => c.id === draggedCrewId);
    if (crewIndex === -1) return;
    
    const crewMember = sourceShip.crew.splice(crewIndex, 1)[0];
...
// ============================================================================
// SHIP DRAG AND DROP (REORDER)
// ============================================================================

function handleShipDragStart(event, shipId) {
    draggedShipCardId = shipId;
    event.dataTransfer.effectAllowed = 'move';
    event.target.style.opacity = '0.6';
}

function handleShipDragEnd(event) {
    draggedShipCardId = null;
    event.target.style.opacity = '1';
}
...
function handleShipDrop(event, targetShipId) {
    event.preventDefault();
    if (draggedShipCardId === null) return;
    if (draggedShipCardId === targetShipId) return;

    const sourceIndex = ships.findIndex(s => s.id === draggedShipCardId);
    const targetIndex = ships.findIndex(s => s.id === targetShipId);
    if (sourceIndex === -1 || targetIndex === -1) return;

    const [moved] = ships.splice(sourceIndex, 1);
    ships.splice(targetIndex, 0, moved);
