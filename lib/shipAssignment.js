    ...
    container.innerHTML = ships.map(ship => `
        <div class="ship-card mb-4 rounded-lg border border-gray-600 bg-gray-800 p-4">
            ...
            <div class="mb-4 flex flex-col gap-3 md:flex-row md:items-end md:gap-4">
                ...
                <!-- Add Crew Button -->
                <button onclick="addCrewMember(${ship.id})" class="rounded-lg border border-green-600 bg-green-700 px-4 py-2 text-sm font-medium text-white hover:bg-green-800">Add Crew</button>
                
                <!-- Remove Ship Button -->
                <button onclick="removeShip(${ship.id})" class="rounded-lg border border-red-600 bg-red-700 px-4 py-2 text-sm font-medium text-white hover:bg-red-800">Remove</button>
            </div>

            <!-- Crew List -->
            <div class="crew-list flex flex-col gap-2">
                ${ship.crew.map(crew => `
                    <div class="flex flex-wrap gap-2 rounded-lg bg-gray-700 p-2 text-sm">
                        <!-- Position -->
                        <select onchange="updateCrewPosition(${ship.id}, ${crew.id}, this.value)" class="flex-shrink-0 min-w-[60px] rounded-md border border-gray-600 bg-gray-600 px-2 py-1 text-center text-xs">
                            <option value="">-</option>
                            ${[1, 2, 3, 4, 5, 6, 7, 8, 9].map(num => `<option value="${num}" ${crew.position === num ? 'selected' : ''}>${num}</option>`).join('')}
                        </select>
                        
                        <!-- Role -->
                        <select onchange="updateCrewRole(${ship.id}, ${crew.id}, this.value)" class="flex-shrink-0 rounded-md border border-gray-600 bg-gray-600 px-2 py-1 text-xs">
                        ...
                        <!-- Discord ID -->
                        <input type="text" placeholder="Discord ID" value="${crew.discordId || ''}" onchange="updateCrewDiscordId(${ship.id}, ${crew.id}, this.value)" class="flex-1 min-w-[120px] rounded-md border border-gray-600 bg-gray-600 px-2 py-1 text-xs">
                        
                        <!-- Name -->
                        <input type="text" placeholder="Name" value="${crew.name || ''}" onchange="updateCrewName(${ship.id}, ${crew.id}, this.value)" list="crew-name-suggestions" class="flex-1 min-w-[120px] rounded-md border border-gray-600 bg-gray-600 px-2 py-1 text-xs">
                        
                        <!-- Comment -->
                        <input type="text" placeholder="Comment" value="${crew.comment || ''}" onchange="updateCrewComment(${ship.id}, ${crew.id}, this.value)" class="flex-1 min-w-[120px] rounded-md border border-gray-600 bg-gray-600 px-2 py-1 text-xs">
                        
                        <!-- Remove Button -->
                        <button onclick="removeCrewMember(${ship.id}, ${crew.id})" class="flex-shrink-0 rounded-md border border-red-600 bg-red-700 px-2 py-1 text-xs text-white hover:bg-red-800">Remove</button>
                    </div>
                `).join('')}
            </div>
        </div>
