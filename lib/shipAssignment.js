...
function onCrewNameChange(shipId, crewId, name) {
    ...
    // Find the team member with this name
    let teamMember = null;
    if (typeof getTeamMembers === 'function') {
        const teamMembers = getTeamMembers();
        teamMember = teamMembers.find(m => m.name === name);
    }
    
    // Auto-fill Discord ID if found
    if (teamMember && teamMember.discordId) {
        crew.discordId = teamMember.discordId;
    }
    
    // Auto-fill Role if found
...
function updateCrewNameDatalist() {
    ...
    if (typeof getTeamMembers === 'function') {
        const teamMembers = getTeamMembers();
        teamMemberData = teamMembers.map(m => ({
            name: m.name,
    ...
    if (leadNameInput && leadNameInput.value.trim()) {
        const existingTeamMember = teamMemberData.find(t => t.name === leadNameInput.value.trim());
    ...
    let teamMemberNames = teamMemberData.map(t => t.name);
    let allNames = [...new Set([...teamMemberNames, ...cachedNames])];
    
    let allDiscordIds = teamMemberData.map(t => t.discordId).filter(id => id);
    ...
    const availableDiscordIds = allDiscordIds.filter(id =>
        !assignedDiscordIds.has(id)
    );
    
    datalist.innerHTML = availableNames.map(name => `<option value="${name}">`).join("");
    discordDatalist.innerHTML = availableDiscordIds.map(id => `<option value="${id}">`).join("");
}
            ...
            <div class="crew-list flex flex-col gap-2">
                ${ship.crew.map(crew => `
                    <div class="crew-member" style="display: grid; grid-template-columns: auto auto 1fr 1fr 1fr auto; gap: 0.75rem; align-items: center; padding: 0.75rem; background: var(--surface); border: 1px solid var(--border); border-radius: 8px;">
                        ...
                        <!-- Name -->
                        <input type="text" placeholder="Name" value="${crew.name || ''}" onchange="onCrewNameChange(${ship.id}, ${crew.id}, this.value)" list="crew-name-suggestions" class="crew-input">
