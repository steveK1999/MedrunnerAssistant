...
//           position?: 'top'|'center', borderStyle?: 'none'|'glow', color?: '#rrggbb' }
export function showAlertOverlay(configOrIndex = 0, durationMs = 3000) {
    ...
    if (typeof configOrIndex === 'object' && configOrIndex !== null) {
        ...
        borderStyle = ['none','glow'].includes(configOrIndex.borderStyle) ? configOrIndex.borderStyle : 'glow';
        color = configOrIndex.color || '#ff0000';
    } else {
    ...
    const r = hexMatch ? parseInt(hexMatch[1], 16) : 255;
    const g = hexMatch ? parseInt(hexMatch[2], 16) : 0;
    const b = hexMatch ? parseInt(hexMatch[3], 16) : 0;
...
foreach ($screen in $screens) {
    ...
    $form.TopMost = $true
    # Use a transparent key color that we won't draw - this makes the entire background transparent
    ...
    $form.Add_Paint({
        ...
        if ($borderStyle -eq 'glow') {
            for ($i=0; $i -lt 6; $i++) {
                $alpha = 100 - ($i * 15)
                $pen = New-Object System.Drawing.Pen([System.Drawing.Color]::FromArgb([Math]::Max(0,$alpha),$colorR,$colorG,$colorB), 12 - $i)
                $r = $rect
        ...
        $brush = New-Object System.Drawing.SolidBrush([System.Drawing.Color]::FromArgb([Math]::Min(255,[Math]::Max(0,$global:textAlpha)),$colorR,$colorG,$colorB))
        $text = 'ALERT'
