...
function updateCrewName(shipId, crewId, name) {
    ...
    if (ship) {
        ...
        if (crew) {
            ...
            if (matchedMember) {
                if (!crew.discordId && matchedMember.discordId) {
...
function updateCrewNameDatalist() {
    ...
    const allNames = Array.from(new Set([...teamNames, ...cachedNames]));
    const availableNames = allNames.filter(name => !assignedNames.has(normalizeName(name)));
    ...
    const discordOptions = new Set();
    teamMembers.forEach((member) => {
        if (member.discordId) {
            discordOptions.add(member.discordId);
        }
    });

    discordOptions.forEach((discordId) => {
        if (!assignedDiscordIds.has(discordId)) {
            const option = document.createElement("option");
            option.value = discordId;
            discordDatalist.appendChild(option);
        }
    });
}
