...
/**
 * Loads workflow from localStorage (supports both old and new format)
 */
function loadWorkflow() {
    try {
        // Try to load from new format first (workflows list + current workflow ID)
        const currentWorkflowId = typeof localStorage !== 'undefined' ? localStorage.getItem("CURRENT_WORKFLOW_ID") : null;
        const workflowsList = typeof localStorage !== 'undefined' ? localStorage.getItem("WORKFLOWS_LIST") : null;
        
        if (currentWorkflowId && workflowsList) {
        ...
        const stored = typeof localStorage !== 'undefined' ? localStorage.getItem(WORKFLOW_STORAGE_KEY) : null;
        if (stored) {
...
function triggerWorkflow(triggerType) {
    ...
    // Check if workflow has pages
    const hasPages = workflow.pages && workflow.pages.length > 0;
    
    if (hasPages) {
        // Open workflow window if pages exist
        if (window.electronAPI && window.electronAPI.send) {
            console.log("Opening workflow window with pages");
            window.electronAPI.send('open-workflow-window', workflow);
        }
    } else {
        // If no pages, just send triggered event for compatibility
        if (window.electronAPI && window.electronAPI.send) {
